"""
Management command para criar o Universo Literário de Tolkien.
"""
...
class Command(BaseCommand):
    help = 'Cria o Universo Literário de Tolkien no banco de dados'

    def handle(self, *args, **options):
        # Buscar autor Tolkien
        try:
            tolkien = Author.objects.get(name__icontains='tolkien')
            self.stdout.write(self.style.SUCCESS(f"✅ Autor encontrado: {tolkien.name} (ID: {tolkien.id})"))
        except Author.DoesNotExist:
            self.stdout.write(self.style.ERROR("❌ Autor Tolkien não encontrado!"))
            return
        ...
        if created:
            self.stdout.write(self.style.SUCCESS(f"✅ Universo criado: {universe.title} (ID: {universe.id})"))
        else:
            self.stdout.write(self.style.WARNING(f"ℹ️ Universo já existe: {universe.title} (ID: {universe.id})"))

        self.stdout.write(f"\n🔗 Acesse no admin: /admin/core/literaryuniverse/{universe.id}/change/")
        self.stdout.write(f"🌐 Página pública: /universo/{universe.slug}/")
